...
fn build_apple_intelligence_bridge() {
    ...
    let force_stub = env::var("HANDY_NO_APPLE_INTELLIGENCE").is_ok();
    let source_file = if has_foundation_models && !force_stub {
        println!("cargo:warning=Building with Apple Intelligence support.");
        REAL_SWIFT_FILE
    } else {
        println!("cargo:warning=Apple Intelligence SDK not found or disabled. Building with stubs.");
        STUB_SWIFT_FILE
    };
